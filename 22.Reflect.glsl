...
// 相机视点位
#define TIME iTime*.5
#define CAMERA_POS mat3(cos(TIME),0,-sin(TIME),0,1,0,sin(TIME),0,cos(TIME))*vec3(5,1,0)
// 相机目标点
...
RayMarchData raymarch(in vec3 ro,in vec3 rd)
{
    float t=TMIN;
    RayMarchData rm;
    rm=RayMarchData(ro,false,vec3(0.),vec3(0.),0);
    for(int i=0;i<RAYMARCH_STEP&&t<TMAX;i++)
    {
        ...
        if(d<PRECISION)
        {
            vec3 n=calcNormal(ray);
            vec3 color=getColor(ray);
            rm=RayMarchData(ray,true,color,reflect(rd,n),minObj);
            rm.color=addlight(ray,rm.color);
            break;
        }
        t+=d;
    }
...
vec3 render(vec2 uv,vec2 px,vec2 py)
{
    ...
    //     }
        color=rm.color;
        // 打光
        // color=addlight(rm.pos,getColor(rm.pos));
        color*=calcAO(rm.pos,calcNormal(rm.pos));
        if(rm.crash&&rm.obj==1){
            // 反射数据
            RayMarchData rmNext=raymarch(rm.pos,rm.reflectnormal);
            if(rmNext.crash){
                color=color*.4+rmNext.color*.6;
            }
        }
    // }
